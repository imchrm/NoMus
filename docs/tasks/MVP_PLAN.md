...
### 1. Backend — NMservices (FastAPI)

**Стек:** Python 3.11+, FastAPI, SQLAlchemy 2.0 (async), PostgreSQL, Alembic, httpx, Pydantic v2

#### База данных

| Таблица | Ключевые поля | Связи |
|---------|---------------|-------|
| `users` | id, phone_number (unique), telegram_id (unique), language_code, created_at, updated_at | → orders (1:N, cascade delete) |
| `services` | id, name, description, base_price, duration_minutes, is_active, created_at, updated_at | → orders (1:N) |
| `orders` | id, user_id (FK), service_id (FK), status, notified_status, total_amount, address_text, scheduled_at, notes, created_at, updated_at | ← users, ← services |

**Статусы заказа:** `pending` → `confirmed` → `in_progress` → `completed` / `cancelled`

#### API — Клиентские endpoints (авторизация: `X-API-Key`)

| Метод | Путь | Назначение |
|-------|------|------------|
| POST | `/users/register` | Регистрация пользователя (phone, telegram_id, language_code) |
| GET | `/users/by-telegram/{id}` | Получение пользователя по Telegram ID |
| PATCH | `/users/{id}/language` | Обновление языка пользователя |
| POST | `/orders` | Создание заказа (user_id, service_id, address_text) |
| GET | `/orders/active` | Список активных заказов пользователя (`{orders: [...]}`) |
| GET | `/orders/pending-notifications` | Непрочитанные уведомления о смене статуса |
| POST | `/orders/notifications/ack` | Подтверждение доставки уведомлений |
| GET | `/services` | Каталог активных услуг |
| GET | `/services/{id}` | Детали услуги |

#### API — Админские endpoints (авторизация: `X-Admin-Key`)

| Метод | Путь | Назначение |
|-------|------|------------|
| GET/POST/DELETE | `/admin/users`, `/admin/users/{id}` | CRUD пользователей (пагинация, сортировка, поиск по ID/телефону, фильтр по дате) |
| GET | `/admin/users/{id}/orders` | Заказы пользователя |
| GET/POST/PATCH/DELETE | `/admin/orders`, `/admin/orders/{id}` | CRUD заказов (фильтр по статусу, дате, user_id; сортировка) |
| GET/POST/PATCH/DELETE | `/admin/services`, `/admin/services/{id}` | CRUD услуг (soft delete через is_active=False) |
| GET | `/admin/stats` | Статистика: кол-во пользователей, заказов, услуг; заказы по статусам |

#### Сервисы

| Сервис | Назначение |
|--------|------------|
| **TelegramNotifier** | Push-уведомления в Telegram при смене статуса заказа (локализованные шаблоны на 3 языках, форматирование цен) |
| **AuthService** | Регистрация, SMS-верификация (stub: код 1234), привязка telegram_id к phone |
| **OrderService** | Создание заказа, валидация услуги, копирование цены, процессинг оплаты (stub) |

---

### 2. Telegram-бот — NoMus (aiogram 3.x)

**Стек:** Python 3.11+, aiogram 3.x, httpx, Pydantic v2
**Архитектура:** DDD (Domain → Application → Infrastructure → Presentation)

#### Пользовательские сценарии

**Регистрация:**
- Выбор языка (🇺🇿 🇬🇧 🇷🇺) → Пользовательское соглашение → Геолокация → Телефон (share contact) → SMS-код (1234) → Готово
- GPS-координаты сохраняются при регистрации

**Создание заказа:**
- 🛒 Сделать заказ → Каталог услуг из API (inline-кнопки: название + цена + длительность) → Ввод адреса → Summary + [Подтвердить] / [Отмена] → `POST /orders` → «Заказ #N создан!»

**Мои заказы:**
- 📋 Мои заказы → `GET /orders/active` → Текстовый список (номер, услуга, сумма, адрес, локализованный статус) или «Нет активных заказов»

**Настройки:**
- ⚙️ Настройки → Inline-меню: Язык / Профиль / О сервисе
- Смена языка обновляет reply-клавиатуру

**Уведомления:**
- Push: NMservices отправляет в Telegram при смене статуса через Admin Panel
- Pull: `NotificationMiddleware` проверяет пропущенные уведомления при каждом взаимодействии
...
| Команда | Действие |
|---------|----------|
| `/start` | Определение статуса пользователя, выбор языка или главное меню |
| `/cancel` | Отмена текущего действия, возврат в главное меню |
| `/language` | Выбор языка (inline-клавиатура) |

#### Техническая реализация

| Компонент | Описание |
|-----------|----------|
| **EmojiPrefixEquals** | Фильтр для reply-кнопок: точное совпадение + fallback без эмодзи |
| **FSM `is_registered`** | Флаг в FSMContext для быстрой проверки регистрации без обращения к storage |
| **L10nMiddleware** | Определение языка (storage → Telegram API → ru) и инъекция `lexicon` в каждый handler |
| **NotificationMiddleware** | Pull-проверка уведомлений при каждом взаимодействии пользователя |
| **ServiceLayerMiddleware** | DI: инъекция `auth_service`, `order_service` в handlers |
| **ServiceFactory** | Создание сервисов по конфигурации: stub/remote SMS, stub/remote payment, memory/remote storage |
| **RemoteStorage** | Write-behind cache: локальный кеш + flush() синхронизирует с API |

#### Локализация

- **3 языка:** русский (ru), узбекский (uz), английский (en)
- **~45 ключей** в `messages.yaml`: приветствия, регистрация, заказ, настройки, статусы, ошибки
- **Pydantic-модель** `Messages` с валидацией непустых значений

#### Режимы работы

| Режим | Конфигурация | Что используется |
|-------|-------------|------------------|
| `development` | `ENV=development` | MemoryStorage, SmsStub (код 1234), PaymentStub, stub-каталог услуг |
| `development-remote` | `ENV=development-remote` | RemoteStorage → NMservices API, SmsRemote, PaymentRemote |
| `staging` / `production` | `ENV=staging\|production` | Полная интеграция с PostgreSQL через NMservices |

---

### 3. Admin Panel — NMservices-Admin (React)
...
| Раздел | Возможности |
|--------|-------------|
| **Dashboard** | Статистика: кол-во пользователей, заказов, услуг; заказы по статусам |
| **Услуги** | Список (фильтрация, сортировка), просмотр, создание, редактирование, деактивация (soft delete) |
| **Заказы** | Список (фильтр по статусу, дате, user_id), просмотр, создание (с autocomplete пользователя), редактирование (статус → push-уведомление в Telegram) |
| **Пользователи** | Список (поиск по ID/телефону, фильтр по дате), просмотр профиля, создание, expand-панель с заказами |
