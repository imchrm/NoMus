...
Здесь вы описываете, что такое ваш бизнес, не думая о базах данных или ботах.

    Пример: Класс Order содержит поля address, tariff, status. Здесь же правила: "нельзя создать заказ без тарифа".

    Почему это DDD: Этот слой не зависит ни от чего внешнего.

2. Application (Приложение) — "Сценарии"

Здесь живет логика процессов, описанных в PoC.
...
Для этапа PoC это критически важный слой. Здесь вы создаете фейковые сервисы, как указано в ТЗ.

    database/memory_storage.py: Вместо PostgreSQL вы используете здесь словарь dict() или список в оперативной памяти, который сбрасывается при перезапуске.
...
Этот слой отвечает за ввод/вывод. В вашей задаче две точки входа:
...
View (keyboards/ и lexicon/): Бот отправляет ответ, используя заготовленные тексты и клавиатуры (например, кнопку "Оплатить 300 руб.").

B. API Service (src/presentation/api/) Согласно плану PoC, у вас должен быть отдельный веб-сервис (бэкенд), который принимает запросы.

    В папке api/routes/ вы реализуете эндпоинты /register и /create_order.
