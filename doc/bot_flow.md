...
4.  `->` **common.cmd_start** (Handler)
    ...
    *   `->` **IUserRepository.save_or_update_user** (Infrastructure - Создание/Обновление пользователя с дефолтными настройками)
...
3.  `->` **common.process_lang_select** (Handler)
    *   `->` **IUserRepository.update_user_language** (Infrastructure - Сохранение выбранного языка в БД)
...
4.  `->` **User** (видит предложение принять условия)
   ...
   > **Примечание:** Если пользователь игнорирует кнопку "Принять условия", он не получает доступа к главному меню.
...
11. `->` **registration.process_code** (Handler)
    ...
    *   `->` Создание сущности **User** (сбор данных: phone, geo)
    *   `->` **AuthService.user_repo.save_or_update_user** (Infrastructure / Repo - Сохранение пользователя с телефоном)
    *   `->` `Message.answer` ("Регистрация успешно завершена!")
    *   **Переход к заказу:**
        *   `->` **IUserRepository.get_user_language** (Получение языка)
...
2.  `->` **common.cmd_cancel** (Handler в `presentation.bot.handlers.common.py`)
    ...
    *   `->` **common.get_start_kb** (Генерация главной клавиатуры)
