"""My Order handler: shows the user's active order."""
...
@router.message(TextEquals("my_order_button"))
async def show_my_order(
    ...
    order = await order_service.get_active_order(message.from_user.id)
    ...
    text = lexicon.my_order_title.format(
