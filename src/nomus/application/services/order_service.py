...
class OrderService:
    ...
    async def get_active_orders(
        ...
        """
        Получает список активных заказов пользователя (pending/confirmed/in_progress).

        Returns:
            Список данных заказов (может быть пустым)
        """
        ...
        try:
            ...
            log.error("Failed to fetch active orders: %s", e)
            return []
